...
class RatingRetriever():
    ...
    #TODO: separate out grubhub restaurant pull into own method () -> method returns all GH restuarants as objects
    #pull grubhub restaurants

    def pull_grubhub_restaurants():
        restaurants = []
        while True:
            ...
            for grubhub_restaurant in search_results.results:
                counter+=1
                restaurant = RestaurantFactory().from_grubhub_restaurant(grubhub_restaurant)
    ...
    #populare restaurant object with known sourc info
    #iterate through restaurants and populate yelp rating info in object
    # for each restaurant, check by phone, and if not found, then check by name, pick best match

    #better to pass in set of main business objects rather than names
    def retrieve_yelp_ratings(self, businesses):
        for business in businesses:
            yelp_api_client.search_matches(name=business.name, city=business.city)
            #print("Checking restaurant {} with id {}".format(grubhub_restaurant.name, grubhub_restaurant.restaurant_id))

            #move phone number formatting as function to phone number class
            phone_number = "+{}{}".format(business.phone_number.country_code, grubhub_restaurant.phone_number.phone_number)
            yelp_businesses_matches = self.yelp_api_client.search_by_phone(phone_number)
            if not yelp_businesses_matches or not yelp_businesses_matches.businesses or len(yelp_businesses_matches.businesses) < 1:
                #print("Error! No Yelp businesses inside of matches: GH: {}".format(grubhub_restaurant.name))
                yelp_businesses_matches = self.yelp_api_client.search_matches(grubhub_restaurant.name, address.street_address,
